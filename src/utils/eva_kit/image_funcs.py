...
from src.utils.image_preprocess import calc_mean_std

def get_mif_normalization_bounds(
    image_paths,
    channel_mask,
    ...
    """
    Extract per-channel lower/upper percentile bounds from several MIF images.

    Returns:
        lower_bounds, upper_bounds  # both shape (C,)
    """
    channel_values = None
    ...
    for path in image_paths:
        ...
        for c in range(C):
            ...
            if vals.size > 0:
                channel_values[c].append(vals)

    lower_bounds = np.zeros(len(channel_values), dtype=np.float32)
    upper_bounds = np.ones(len(channel_values), dtype=np.float32)

    for c, vals_list in enumerate(channel_values):
        if len(vals_list) == 0:
            continue

        vals = np.concatenate(vals_list)
        lower_bounds[c] = np.percentile(vals, lower_percentile)
        upper_bounds[c] = np.percentile(vals, upper_percentile)

    return lower_bounds, upper_bounds

def normalize_mif_image(
    img,
    lower_bounds,
    upper_bounds,
    channel_first=True,
    ...
    """
    Clip and normalize one MIF image to [0, 1] using precomputed bounds.
    """
    ...
    lower_bounds = np.asarray(lower_bounds, dtype=np.float32).reshape(1, 1, -1)
    upper_bounds = np.asarray(upper_bounds, dtype=np.float32).reshape(1, 1, -1)

    img = np.clip(img, lower_bounds, upper_bounds)
    img = (img - lower_bounds) / (upper_bounds - lower_bounds + 1e-8)
...
def cell_seg(img_path,
    ...
    raw = TiffFile(img_path).asarray()[channel_mask,:,:]
    mask = TiffFile(mask_path).asarray()

    lower_bounds, upper_bounds = properties
    raw = normalize_mif_image(raw, lower_bounds, upper_bounds)
    ...
    for raw_patch, mask_patch, i in patch_generator(raw,
                                                      mask,
                                                      cell_cutout,
                                                      df):
        raw_patch= torch.tensor(raw_patch)
        raw_patch = raw_patch.unsqueeze(0) # (C,H,W) -> (1,C,H,W)
        if cell_cutout != 224:
            raw_patch = F.interpolate(
                    raw_patch,
                    size=(224,224),
                    mode="bilinear",
                    align_corners=False
...
def image_preprocess(path,
    ...
    channel_mask = np.array([True]*len(channel_names))
    ...
    for img_name in img_names:   
        ...
        properties = get_mif_normalization_bounds(img_paths, channel_mask)
