...
from src.utils.kronos_kit.dino_head import DINOHead
from src.utils.kronos_kit.mlp import Mlp
from src.utils.kronos_kit.patch_embed import PatchEmbed
from src.utils.kronos_kit.swiglu_ffn import SwiGLUFFN, SwiGLUFFNFused
...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        self.patch_embed = embed_layer(img_size=img_size, patch_size=patch_size, stride_size=stride_size, in_chans=1, embed_dim=embed_dim)
        num_patches = self.patch_embed.num_patches

        # -- modality embedding
        self.marker_embed = get_1d_sincos_marker_embed(embed_dim, num_markers, cls_token=False)
    ...
    def prepare_tokens_with_masks(self, x, masks=None, marker_ids=None):
        ...
        marker_embed = torch.from_numpy(self.marker_embed).float().unsqueeze(0).to(device=x.device, dtype=x.dtype)
        ...
        pos_embed = torch.cat((pos_embed[:, 0, :].unsqueeze(0), pos_embed[:, 1:, :].repeat(1, num_marker, 1)), dim=1)
        x = x + pos_embed
    ...
    def forward_features_list(self, x_list, masks_list, marker_ids_list):
        x = [self.prepare_tokens_with_masks(x, masks, marker_ids) for x, masks, marker_ids in zip(x_list, masks_list, marker_ids_list)]
    ...
    def forward(self, x, masks=None, marker_ids=None, is_training=False):
        ...
        if is_training:
            ...
            patch_features = ret["x_norm_clstoken"]
            patch_token_features = ret["x_norm_patchtokens"].reshape(B, num_marker, tokens_per_row, tokens_per_col, self.embed_dim)
    ...
    def get_intermediate_layers(
        ...
        if self.chunked_blocks:
            outputs = self._get_intermediate_layers_chunked(x, marker_ids, n)
        else:
            outputs = self._get_intermediate_layers_not_chunked(x, marker_ids,  n)
        if norm:
